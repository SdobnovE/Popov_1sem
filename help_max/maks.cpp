...
int main(int argc, char*argv[])// C N M MU
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        b[0]= -H[i*(M+1)+0] * V[i*(M+1)+1] / (2.0*h) - (1/(2.0*h)) * (H[i*(M+1)+2] * V[i*(M+1)+2]
                -2*H[i*(M+1)+1]*V[i*(M+1)+1]+H[i*(M+1)+0]*V[i*(M+1)+0] / (h * h)
                -0.5*(H[i*(M+1)+3]*V[i*(M+1)+3]-2*H[i*(M+1)+2]*V[i*(M+1)+2]+H[i*(M+1)+1]*V[i*(M+1)+1]) / (h * h)
        ...
        for (int i = 0; i < M + 1; i++)
            printf("down %e ", a[i + 2*(M+1)]);
        printf("\n");
        for (int i = 0; i < M + 1; i++)
            printf("mid %e ", a[i + 1*(M+1)]);
        printf("\n");
        for (int i = 0; i < M + 1; i++)
            printf("up %e ", a[i + 0*(M+1)]);
        printf("\n");
